...
os.environ["CUDA_VISIBLE_DEVICES"] = "5"  # Only GPU 0 is visible to this program
...
def cross_validate(technique, base_dir, k_folds):
    ...
    with open(f"{base_dir}/image_classification/vgg16/metrics/{technique}_{chosen_body_part}_metrics.txt", "w") as file:
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(videos, video_labels_numeric)):
            ...
            # Model setup
            base_model = VGG16(include_top=False, input_shape=(img_height, img_width, 3), pooling='avg', weights='imagenet')
            ...
            model_save_path = f"{base_dir}/image_classification/vgg16/models/{technique}_{chosen_body_part}_fold_{fold+1}_model_new.keras"
