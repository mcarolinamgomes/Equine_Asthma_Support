...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # Only GPU 0 is visible to this program
...
def prepare_image(frame_path, img_height, img_width):
    try:
        ...
        img_array = preprocess_input(img_array)  # Additional preprocessing for EfficientNet
        return img_array
    except Exception as e:
        print(f"Error processing file {frame_path}: {str(e)}")
        return None
...
def get_video_frame_paths(technique_dir):
    ...
    for body_part in os.listdir(technique_dir):
        if body_part != chosen_body_part:
            ...
            continue
        print('same body part',chosen_body_part)
...
def process_and_load_images(frame_paths, img_height, img_width, class_labels):
    def generator():
        for frame_path in frame_paths:
            img_array = prepare_image(frame_path, img_height, img_width)
            if img_array is not None:  # Check if img_array is not None
                parts = frame_path.split(os.sep)
                class_label = parts[-3]
                if class_label in class_labels:
                    # Ensure img_array is converted to uint8 after preprocessing
                    img_array = (img_array * 255).astype(np.uint8)
                    yield (img_array, class_labels.index(class_label))
                else:
                    print(f"Unrecognized label {class_label} found in {frame_path}")
            else:
                print(f"Failed to process image at {frame_path}")
...
def cross_validate(technique, base_dir, k_folds):
    ...
    with open(f"{base_dir}/image_classification/efficientnet/metrics/{technique}_{chosen_body_part}_metrics.txt", "w") as file:
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(videos, video_labels_numeric)):
            ...
            # Model setup
            base_model = EfficientNetB0(include_top=False, input_shape=(img_height, img_width, 3), pooling='avg', weights='imagenet')
            ...
            model_save_path = f"{base_dir}/image_classification/efficientnet/models/{technique}_{chosen_body_part}_fold_{fold+1}_model_new.keras"
